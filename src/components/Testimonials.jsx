...
const Testimonials = () => {
  ...
  return (
    <section className="py-24 bg-japan-white relative overflow-hidden">
      <div className="absolute inset-0 bg-japan-red/5"></div>
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        ...
        >
          ...
          <div className="flex justify-center items-center gap-2">
            <div className="flex gap-1">
              {[...Array(5)].map((_, i) => (
                <Star key={i} className="w-5 h-5 fill-japan-red text-japan-red" />
              ))}
            </div>
        ...
        <div className="relative max-w-4xl mx-auto">
          <AnimatePresence mode="wait">
            ...
            >
              <Quote className="text-japan-red/20 w-16 h-16 absolute top-4 right-4" />
              <div className="flex flex-col items-center text-center">
                <img
                  src={testimonials[current].image}
                  alt={testimonials[current].name}
                  className="w-20 h-20 rounded-full object-cover border-4 border-japan-red mb-4"
                />
                ...
                <p className="text-japan-red">{testimonials[current].program}</p>
          ...
          <button
            ...
            className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-12 bg-white rounded-full p-2 shadow-lg hover:bg-japan-red hover:text-white transition-all duration-300"
          >
            <ChevronLeft size={24} />
          </button>
          <button
            onClick={() => setCurrent((prev) => (prev + 1) % testimonials.length)}
            className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-12 bg-white rounded-full p-2 shadow-lg hover:bg-japan-red hover:text-white transition-all duration-300"
          >
          ...
          {testimonials.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                current === index ? 'bg-japan-red w-8' : 'bg-japan-red/30'
              }`}
            />
          ))}
        </div>
